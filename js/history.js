...
function getBodyExchangePersonalText(event, otherName) {
  const source = getEventSource(event);
  if (source === "落雷" || event.text.includes("落雷")) {
    return otherName ? `雷に打たれて、${otherName}と身体が入れ替わる。` : toPersonalTense(event.text);
  }
  if (source === "秘宝") {
    return otherName ? `秘宝の力により、${otherName}と身体が入れ替わる。` : toPersonalTense(event.text);
  }
  if (event.text.includes("境")) {
    return otherName ? `村の境が揺らぎ、${otherName}と身体が入れ替わる。` : toPersonalTense(event.text);
  }
  return otherName ? `奇跡の光により、${otherName}と身体が入れ替わる。` : toPersonalTense(event.text);
}
...
// 個人史は現在形で綴る。保存済みの本文しかない記録は、文末だけを現在形へ直す。
const PERSONAL_TENSE_ENDINGS = [
  ["った。", "る。"], ["んだ。", "む。"], ["した。", "する。"], ["けた。", "ける。"],
  ["れた。", "れる。"], ["ちた。", "ちる。"], ["えた。", "える。"], ["めた。", "める。"]
];

function toPersonalTense(text) {
  const cleaned = cleanRecordText(text);
  const ending = PERSONAL_TENSE_ENDINGS.find(([past]) => cleaned.endsWith(past));
  return ending ? cleaned.slice(0, -ending[0].length) + ending[1] : cleaned;
}
...
function getPersonalHistoryText(event, personName, personId = null) {
  ...
  switch (event.type) {
    ...
    case HISTORY_EVENT_TYPES.DRYAD_FRUIT:
      return toPersonalTense(event.text).replace(`${personName}が`, "").trim() || "ドライアドの身体となる。";
    case HISTORY_EVENT_TYPES.HEADMAN_ELECTION:
      return event.title.includes("続ける") ? "里長を続けることになる。" : "里長に選ばれる。";
    case HISTORY_EVENT_TYPES.MARRIAGE:
      return otherName ? `${otherName}と夫婦となる。` : toPersonalTense(event.text);
    case HISTORY_EVENT_TYPES.LOVER:
      return otherName ? `${otherName}と恋人になる。` : toPersonalTense(event.text);
    case HISTORY_EVENT_TYPES.SOCIAL_RELATION: {
      ...
      if (relation === "趣味仲間") {
        return otherName
          ...
          : toPersonalTense(event.text);
      }
      return otherName ? `${otherName}と${relation}になる。` : toPersonalTense(event.text);
    }
    case HISTORY_EVENT_TYPES.HOBBY_AWAKENING: {
      const hobby = event.tags[1] || "";
      return hobby ? `${hobby}の趣味に目覚める。` : toPersonalTense(event.text);
    }
    case HISTORY_EVENT_TYPES.PREGNANCY: {
    ...
    case HISTORY_EVENT_TYPES.BIRTH: {
      ...
      return toPersonalTense(event.text);
    }
    case HISTORY_EVENT_TYPES.VILLAGER_JOIN: {
      const source = normalizeJoinSource(getEventSource(event));
    ...
    case HISTORY_EVENT_TYPES.EPIDEMIC:
      return "病に倒れる。";
    case HISTORY_EVENT_TYPES.MYTHIC_EVENT:
      return toPersonalTense(event.text).replace(`${personName}が`, "").trim() || toPersonalTense(event.text || event.title);
    default:
      return toPersonalTense(event.text || event.title);
  }
}
