...
// 生成時の既定の勤勉レンジ。訪問者や襲撃者はこれを引き継ぐ。
const DEFAULT_IND_RANGE = [5, 25];
// 初期村人だけは、働けない者ばかりで詰まないように下限を上げる。
const INITIAL_VILLAGER_IND_RANGE = [8, 25];
...
export function createInitialVillagers() {
  ...
  for (let i = 0; i < totalCount; i++) {
    ...
    if (isMale) {
      ...
      initRandomParams(male, { indRange: INITIAL_VILLAGER_IND_RANGE });
      ...
      initRandomParams(female, { indRange: INITIAL_VILLAGER_IND_RANGE });
 ...
 * パラメータ初期化
 */
export function initRandomParams(v, { indRange = DEFAULT_IND_RANGE } = {}) {
  const [indMin, indMax] = indRange;
  // 性別別にある程度初期値の振れ幅を設定
  if (v.bodySex === "男") {
    ...
    v.ind = randNormalInRange(indMin, indMax);
