...
- [relationship-simulation-results.md](relationship-simulation-results.md): 標準プレイヤー50村・60か月の好感度と人間関係イベント結果。
...
6. 予備基準測定: 実施中。上位襲撃の概略数値調整に着手済み。正式な1,000回測定と最終手動調整は未実施。

測定基盤は `tools/balance/index.html` から利用します。上位襲撃の概略調整では、ゲーム本体の強版襲撃に限定した敵数・身体能力・精神特性を変更します。敵HPが100を超える場合の切り下げは、測定前提となる不具合として修正しました。
