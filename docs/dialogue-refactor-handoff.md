...
通常の村人会話は `js/dialogue/dialogueEngine.js` が中心になっている。ただし、恋愛成立・結婚・奇跡結果・交換の奇跡など、一部の専用モーダル用セリフはまだ各処理ファイル内に残っている。新しく会話データを追加するときは、対象の表示経路を確認してから置き場所を選ぶ。
...
- `js/miracles.js`
  ...
  - 現時点では、奇跡結果や交換の奇跡の口調別セリフをこのファイル内に持つ。
