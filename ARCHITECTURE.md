...
- `js/autoAssign.js`
  - 「自動割り振り」と「防衛割り振り」の判断を持つ。仕事は `js/domain/jobMath.js` の期待成果を資源軸へ振り分け、資源軸ごとの重みで選ぶ。食料・資材・治安・資金・技術の段階と倍率は `js/autoAssignSettings.js` が持ち、回復・幸福は不足度で連続的に上げる。防衛は `js/raidRules.js` の枠と参加条件に従って前衛・罠・中衛を埋める。防衛割り振りは通常・堅忍・省力の3モードを持ち、作戦会議の3つのボタンから `autoAssignRaidActions(village, { mode })` へ渡す。
- `js/warCouncil.js`
