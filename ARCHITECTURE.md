...
- `js/autoAssign.js`
  - 「自動割り振り」と「防衛割り振り」の判断を持つ。仕事は `js/domain/jobMath.js` の期待成果を資源軸へ振り分け、資源軸ごとの重みで選ぶ。食料・資材・治安・資金・技術の段階と倍率は `js/autoAssignSettings.js` が持ち、回復・幸福は不足度で連続的に上げる。防衛は `js/raidRules.js` の枠と参加条件に従って前衛・罠・中衛を埋める。防衛の自動割り振りは攻勢・堅忍・省力の3方針を持ち、作戦会議の「自動割り振り方針」ボタンから `autoAssignRaidActions(village, { mode })` へ渡す。
- `js/warCouncil.js`
  - 襲撃中の「作戦会議」画面。前衛・中衛・後衛の配置チェック、枠の消化状況、予想ダメージと被弾倍率の表示を扱う。判定と枠は `js/raidRules.js` を通す。
  - 体力・筋力・器用・魔力・知力・勇気は見出しクリックで並び替えできる。全能力（幸福度を含む）は枠行の「全能力表示」で切り替える。
...
- `js/relationships.js`
  - 恋人・配偶者・親子などの関係を正規化し、追加・削除・表示する。関係は `{ prefix, targetId, targetName }` のエントリで保存し、判定は相手ID、表示は記録時の名前スナップショットで行う。好感度 `friendships` と共働き回数 `friendshipStats` のキーも人物ID。仕事仲間の組は、最後に同じ仕事をした月の通し番号を `friendshipStats.lastWorkTogether` に持ち、6か月以上同じ仕事をしないと月次処理で仕事仲間と共働き回数を消す。この記録を持たない旧保存データの仕事仲間は、読込後の最初の月次処理から数え始める。関係配列・好感度・共働き回数は、全ペアの月次処理で重くならないよう一度正規化したものを使い回すため、他のファイルから直接書き換えず、このファイルの関数を通す。読込時の補正のようにエントリを直接書き換えた場合は、`normalizeRelationships` で正規化し直す。
- `js/miracles.js`
...
- `js/data/nameData.js`
  - 村人・訪問者・襲撃者の名前一覧と、性別・種族・肩書から使う一覧を選ぶ規則。名前を増やすときはこのファイルだけを触る。
  - 一般名のほかに、遊牧系（エクイナ・セントール等）と翼人系の専用一覧を持つ。翼人系は訪問者・翼人兵・上位翼人で共通の一覧を引く。村で生まれた翼人は肩書を持たないため、一般名の一覧から名付けられる。
  - 名前の予約は今の村にいる者（村人・訪問者・襲撃者・捕虜）から都度導き、去った者の名前は再び使える。過去帳に残る名は、未使用の名が9以下になるまで自動命名の候補から外す（プレイヤー命名では使える）。専用一覧を使い切った場合、翼人系は `getFallbackNameList` で一般名の一覧へ切り替える。最後まで尽きた一覧では名前2つを「＝」でつなぐ。
  - 一般名の一覧は系統ごとにコメントで区切ってある。名前を足すときは、合う系統の末尾へ加える。
- `js/data/wishData.js`
  - 願望の表示名、発生時と達成時のセリフ、発生率、継続期間を定義する。
- `js/records.js`
  - 台帳の「殿堂」に出す歴代記録。項目の定義、加算・最高値の更新、上位の取得を扱う。
  - 記録は村人ではなく村（`village.villageRecords`）へ精神IDごとに積むため、死亡・出立しても残る。
- `js/ledger.js`
  - 台帳モーダルと、選挙記録・願望・殿堂・経営目標の各画面。村史と過去帳は `js/history.js` の画面を台帳から開く。
...
- `js/saveLoad.js`
  - JSON ファイル保存/読込と localStorage 保存/読込を扱う。ログ（`logs`）は読込後のログ欄へ戻さないため、直近1000行だけを保存する。
- `js/dictionary.js`
  - 用語検索を扱う。
- `js/util.js`
...
配布用zipは `tools/` のスクリプトで作る。どちらも `index.html` / `css` / `js` / `images` だけを入れ、顔原本PNGは含めずアトラスと上記SVG 2枚だけを含める。ゲームから読み込まない `js/data/dialogue/basicConversationLines.js`（旧会話の文面の保管用）も外し、同梱するJSがこれを参照していたら書き出しを止める。zip内の `index.html` からはGoogleアナリティクスのタグを外し、除去できたかを検証する。`--no-images` を付けると `images/` を外した軽いzipになり、画像は手で足す前提になる。zipの書き出しと計測タグの除去は `package_itch.py` に置き、夢現向けはそれを読み込んで使う。

- itch.io向け: `python tools/package_itch.py` → `output/village-of-bacchus-itch.zip`。出力ファイル数が1,000未満であることも検証する。
- フリーゲーム夢現向け: `python tools/package_mugen.py` → `output/village-of-bacchus-mugen-v<版>.zip`。終盤の内容を伏せた `Readme_mugen.txt` を、zip内では `Readme.txt` という名前で同梱する。配布先では未プレイの人が先に読むため、リポジトリの `Readme.txt` とは分けている。出力ファイル名の版は `index.html` のタイトル表記（`ver.1.01` など）から読み、オープニング画面の版表示も同じ表記を使う。版を上げるときは `index.html` の表記を直し、両方の Readme の更新履歴に追記する。

## 訪問者とレア種族

- 訪問者タイプの月次抽選は `js/createVillagers.js` の `VISITOR_TABLES_BY_SCALE` を使う。レア種族枠が選ばれた場合は、同ファイル内でさらにレア種族テーブルを重み付き抽選する。
- 冒険者クエストの4候補は訪問者本人の `adventurerQuestOffers` に保存し、同じ訪問中に会話モーダルを開き直しても再抽選しない。資金500の引き直しだけが例外で、使用済みかどうかは `adventurerQuestRerolled` に持つ。受諾後は `Village.activeAdventurerQuests` へ人物ごと移し、6か月後の月初に通常抽選とは別枠で訪問者へ戻す。
